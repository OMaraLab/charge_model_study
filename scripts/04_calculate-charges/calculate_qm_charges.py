...
class RespCalculator:
    ...
    def __post_init__(self):
        self.conformer_filenames = [
            ...
            for conformer_number in sorted(self.conformer_numbers)
        ]
        self.filenames = Filenames(
            ...
            charge_method_name=self.charge_method_name,
            environment=self.environment,
            root_charge_directory=self.output_directory,
        )

        self.charge_method: QMMethod = ChargeMethod[self.charge_method_name.upper()].value
    ...
    def get_psiresp_job(self):
        orientations = [
            psiresp.Orientation.parse_file(
                filename.esp_orientation_filename)
            for filename in self.conformer_filenames
        ]
        conformers = [
            psiresp.Conformer(
    ...
    def write_esp_surface_constraint_matrix(self):
        ...
        self.filenames.write_to_file(
            self.filenames.surface_constraint_matrix_filename,
    ...
    def get_n_structure_array(self):
        ...
        return np.array([n_conformers] * len(self.offmol.atoms))
    ...
    def calculate_charges(self):
        ...
        respcharges_1.solve()
        self.filenames.write_to_file(
            self.filenames.stage_1_filename,
            respcharges_1.json(),
        )
        ...
        if self.charge_method.stage_2:
            ...
            respcharges_2.solve()
            self.filenames.write_to_file(
                self.filenames.stage_2_filename,
                respcharges_2.json(),
            )
        ...
        pathlib.Path(self.filenames.resp_charges_filename).parent.mkdir(parents=True, exist_ok=True)
...
@click.option(
    "--charge-method-name",
    type=click.Choice(["resp", "resp2", "atb"]),
    required=True,
)
@click.option(
    "--environment",
...
def main(
    ...
    conformer_numbers = list(map(int, conformer_numbers.strip().split()))

    calculator = RespCalculator(
        component_number=component_number,
        conformer_numbers=conformer_numbers,
